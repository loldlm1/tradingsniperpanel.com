...
## API surface
- `POST /api/v1/licenses/verify` (JSON only): params `source`, `email`, `ea_id`, `license_key`, optional `addons` (comma-separated add-on keys), optional `broker_account` payload (`name`, `company`, `account_number`, `account_type`). Success returns `ok`, `plan_interval`, `trial`, `expires_at` (unix timestamp), optional `broker_account`. Errors: `user_not_found`, `ea_not_found`, `license_not_found`, `invalid_key`, `expired`, `trial_disabled`, `addons_required` (includes `required_addons` and `missing_addons` CSVs), `invalid_source`, `invalid_payload`, or `rate_limited`.
- `POST /api/v1/licenses/verify` (JSON only): params `source`, `email`, `ea_id`, `license_key`, optional `addons` (comma-separated add-on keys), required `broker_account` payload (`name`, `company`, `account_number`, `account_type`). Success returns `ok`, `plan_interval`, `trial`, `expires_at` (unix timestamp), optional `broker_account`. Errors: `user_not_found`, `ea_not_found`, `license_not_found`, `invalid_key`, `expired`, `trial_disabled`, `addons_required` (includes `required_addons` and `missing_addons` CSVs), `invalid_source`, `invalid_payload`, `online_limit_reached`, or `rate_limited`.
- `POST /api/v1/licenses/heartbeat` (JSON only): params `source`, `email`, `ea_id`, `license_key`, required `broker_account` (`company`, `account_number`, `account_type`, optional `name`). Success refreshes online lease and returns `ok`, `plan_interval`, `trial`, `expires_at`. Errors mirror verify, including `online_limit_reached`.
