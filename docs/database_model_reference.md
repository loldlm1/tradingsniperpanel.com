...
- `license_lane_magic_numbers`: `license_id`, `source`, `email`, `company`, `account_number`, `account_type`, `magic_number`. Unique lane key: `license_id + source + email + company + account_number + account_type`; `magic_number` is globally unique and positive.
...
- `broker_account_daily_results`: `broker_account_id`, `expert_advisor_id`, `magic_number`, `result_timestamp` (unix seconds), `result_value` (decimal). Uniqueness: one result per UTC day for `broker_account + expert_advisor + magic_number`.
...
- Lane magic assignment: `Licenses::LaneMagicNumberAllocator` creates/reuses secure random lane `magic_number` values for `licenses/verify`.
...
## API surface
- `POST /api/v1/licenses/verify` (JSON only): params `source`, `email`, `ea_id`, `license_key`, optional `addons` (comma-separated add-on keys), required `broker_account` payload (`name`, `company`, `account_number`, `account_type`). Success returns `ok`, `plan_interval`, `trial`, `expires_at` (unix timestamp), required `magic_number`, `granted_addons`, and optional `broker_account`. Errors: `invalid_source`, `invalid_payload`, `user_not_found`, `ea_not_found`, `license_not_found`, `invalid_key`, `expired`, `trial_disabled`, `addons_required` (`required_addons`/`missing_addons`), `online_limit_reached`, `rate_limited`, `internal_error`.
