...
- `broker_account_daily_results`: `broker_account_id`, `expert_advisor_id`, `magic_number`, `result_timestamp` (unix seconds), `result_value` (decimal). Uniqueness: one result per UTC day for `broker_account + expert_advisor + magic_number`; new writes expect the same signed-32-bit-safe range used by lane allocation.
