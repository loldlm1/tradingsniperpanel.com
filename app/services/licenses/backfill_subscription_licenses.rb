module Licenses
  class BackfillSubscriptionLicenses
    Result = Struct.new(
      :dry_run,
      :scanned,
      :eligible,
    ...
    def call
      ...
      candidate_users.find_in_batches(batch_size: batch_size).with_index do |users, batch_index|
        logger.info(
          "[#{self.class.name}] batch=#{batch_index + 1} size=#{users.size} " \
    ...
    attr_reader :ea_id, :tiers, :dry_run, :batch_size, :user_ids, :now, :encoder, :subscription_finder, :status_resolver, :logger
    ...
    def manual_candidate_user_ids
      ManualSubscription
        .active_at(now)
        .joins(:billing_plan)
        .where(billing_plans: { tier: tiers, key: eligible_plan_keys })
        .select(:user_id)
    end

    def eligible_plan_keys
      Billing::SubscriptionCatalog.products.select { |product| tiers.include?(product.tier) }.flat_map(&:plan_keys)
    end

    def eligible_price_ids
      plan_ids = BillingPlan.where(
        tier: tiers,
        key: eligible_plan_keys,
    ...
    def process_user(user, result)
      ...
      logger.error(
        "[#{self.class.name}] failed user_id=#{user.id} error=#{e.class}"
      )
    end
    ...
    def access_context_for(user)
      ...
      return unless product && tiers.include?(product.tier) && product.ea_ids.include?(ea_id)
      ...
      expires_at = source == :manual ? subscription.effective_ends_at : subscription.current_period_end || subscription.ends_at
      return unless expires_at.present? && expires_at > now

      target_ea = ExpertAdvisor.active.find_by(ea_id: ea_id)
      return unless target_ea

      entitled_eas = ExpertAdvisor.subscription_entitlements_for(plan)
      return unless entitled_eas.any? { |expert_advisor| expert_advisor.id == target_ea.id }

      {
        expert_advisor: target_ea,
