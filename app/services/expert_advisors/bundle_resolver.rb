module ExpertAdvisors
  class BundleResolver
    ...
    def call
      ...
      bundle_key = addon_keys.empty? ? "base" : addon_keys.join("__")
      bundle = expert_advisor.expert_advisor_bundles.active.find_by(bundle_key: bundle_key)

      if bundle&.bundle_file&.attached?
        return Result.new(bundle: bundle, bundle_key: bundle_key, addon_keys: addon_keys, missing_bundle: false)
      end

      Result.new(bundle_key: bundle_key, addon_keys: addon_keys, missing_bundle: true)
    end
